...
window.document.title = "MengCDN - Github设置";
...
let AXSW01 = () => {
  ...
  axios.put('/api/v1/cdnSW/0/Github/' + msw11, {
    "value": form.whiteList
  }).then((res) => {
    if (res.data.status == 200) {
      Message.success(res.data.message)
...
let AXSW02 = () => {
  ...
  axios.put('/api/v1/cdnSW/1/Github/' + msw22, {
    "value": form.whiteList
  }).then((res) => {
    if (res.data.status == 200) {
      Message.success(res.data.message)
          ...
          <a-form :model="form" @submit="handleSubmit">
            <a-row>
              <a-col :xs="24" :sm="24" :md="24" :lg="12" :xl="12" :xxl="12">
                <a-form-item field="whiteList" tooltip="用户名/仓库" label="白名单">
