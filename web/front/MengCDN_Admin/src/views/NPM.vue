...
let SelectedKeys: any = ref(["3"]);
emitter.emit("getSelectedKeys", SelectedKeys);
window.document.title = "MengCDN - NPM设置";
...
let AXSW01 = () => {
  ...
  axios.put('/api/v1/cdnSW/0/Npm/' + msw11, {
    "value": form.whiteList
  }).then((res) => {
    if (res.data.status == 200) {
      Message.success(res.data.message)
...
let AXSW02 = () => {
  ...
  axios.put('/api/v1/cdnSW/1/Npm/' + msw22, {
    "value": form.whiteList
  }).then((res) => {
    if (res.data.status == 200) {
      Message.success(res.data.message)
