...
let SelectedKeys: any = ref(["4"]);
emitter.emit("getSelectedKeys", SelectedKeys);
window.document.title = "MengCDN - WordPress|plugin 设置";
...
let AXSW01 = () => {
  ...
  axios.put('/api/v1/cdnSW/0/WPpl/' + msw11, {
    "value": form.whiteList
  }).then((res) => {
    if (res.data.status == 200) {
      Message.success(res.data.message)
...
let AXSW02 = () => {
  ...
  axios.put('/api/v1/cdnSW/1/WPpl/' + msw22, {
    "value": form.whiteList
  }).then((res) => {
    if (res.data.status == 200) {
      Message.success(res.data.message)
